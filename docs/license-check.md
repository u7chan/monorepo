...
`scripts/check_licenses.py` 自体は Python 標準ライブラリのみで動きます。dependency metadata の収集では package manager を外部コマンドとして実行します。Python target の dependency install では `uv sync --python 3.13` を使います。

PR CI では `oven-sh/setup-bun@v2` と `astral-sh/setup-uv@v8` で `bun` / `uv` を用意します。
...
| Python | `uv.lock` | `uv sync --frozen --no-dev --no-install-project --no-install-workspace --python 3.13` |
