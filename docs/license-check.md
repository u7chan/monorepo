...
`scripts/check_licenses.py` 自体は Python 標準ライブラリのみで動きます。dependency metadata の収集では package manager を外部コマンドとして実行します。Python target の dependency install では、`pyproject.toml` の `requires-python` を見て `uv sync --python <version>` の interpreter を選びます。
