...
永続化設定は導入済みだが、再デプロイ後の作業ファイル・Git worktreeの保持確認は未完了。
設定上の期待値と実機検証済みの範囲を混同しないこと。

再デプロイ前にテスト用ファイルとGit worktreeを `/workspace` 内に作り、
再デプロイ後に新しい会話またはサンドボックス内のコマンドで内容と
`git worktree list` を確認する。会話履歴が消えることは失敗条件に含めない。
