...
export type ManagerDialogProps = {
  open: boolean;
  onClose: () => void;
  catalog: Catalog;
...
export function ManagerDialog({
  ...
  useEffect(() => {
    ...
    if (open && !dialog.open) {
      ...
      setEditingId(agentId || catalog.agents[0]?.id || null);
      dialog.showModal();
  ...
  return (
    <dialog
      ref={dialogRef}
      onClose={onClose}
      className="m-auto h-[min(760px,calc(100dvh-28px))] w-[min(760px,calc(100%-28px))] rounded-2xl border border-line bg-panel p-0 text-ink shadow-panel backdrop:bg-black/40 max-nav:h-[min(760px,calc(100dvh-16px))] max-nav:w-[calc(100%-16px)]"
    >
      <div className="flex h-full min-h-0 flex-col">
        {/* ヘッダ */}
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-line px-5 pb-3.5 pt-4 max-nav:grid max-nav:grid-cols-[minmax(0,1fr)]">
          <div>
          ...
          <div className="flex items-center gap-2 max-nav:w-full max-nav:justify-between">
            <div className="flex gap-2">
              <button
                ...
                className="min-h-9 rounded-lg border border-line px-3 text-xs text-ink-soft transition-colors hover:border-accent/50 hover:text-accent max-nav:flex-1"
              >
                インポート
              </button>
              <button
            ...
            <button
              ...
              onClick={onClose}
              aria-label="閉じる"
              className="grid size-8 place-items-center rounded-lg border border-line text-sm text-ink-soft transition-colors hover:border-danger/60 hover:text-danger"
            >
              ×
            </button>
          </div>
        ...
        {/* 本文 */}
        <div className="grid min-h-0 flex-1 grid-cols-1 max-nav:grid-rows-[minmax(120px,24vh)_minmax(0,1fr)] wide:grid-cols-[220px_minmax(0,1fr)]">
          {/* 左: 一覧 */}
          <aside className="grid min-h-0 gap-3 overflow-y-auto border-b border-line px-3 py-3 scrollbar-thin wide:border-b-0 wide:border-r">
            <section className="grid min-h-0 gap-1.5">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-semibold uppercase tracking-widest text-ink-faint">エージェント</span>
                <button
                  type="button"
                  onClick={startNewAgent}
                  aria-label="新しいエージェント"
                  className="grid size-6 cursor-pointer place-items-center rounded-md border border-line text-xs text-ink-soft transition-colors hover:border-accent/50 hover:text-accent"
                >
                  ＋
                </button>
              </div>
              <div className="grid min-h-0 gap-1 overflow-y-auto scrollbar-thin">
            ...
            <section className="grid min-h-0 gap-1.5">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-semibold uppercase tracking-widest text-ink-faint">スキル</span>
                <button
                  type="button"
          ...
          {/* 右: エディタ */}
          <section className="min-h-0 overflow-y-auto px-4 py-4 scrollbar-thin">
            {isAgent ? (
              <form onSubmit={saveAgent} className="grid gap-3">
                ...
                <div className="grid gap-1.5">
                  {catalog.skills.length === 0 ? (
                    <div className="text-[11px] text-ink-faint">スキルがありません。左の＋から作成できます。</div>
                  ) : (
                    catalog.skills.map((skill) => (
              ...
              <form onSubmit={saveSkill} className="grid gap-3">
                <div>
