...
- **譜面定数の収集・再現** → **収集は完了（2026-08-31）**: 全譜面分（`data/maimai_constants.json`・6364 譜面 /
  1479 曲名。poc/constants.json は検証用 12 譜面のまま）。ただし**収集過程に再現性がない**（第三者サイト等からの
  手作業収集・出典名非記載。domain.md『注意・データギャップ』）。残課題は収集の自動化（取り込みツール構想）と
  再取得・更新の手順化。
