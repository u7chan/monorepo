...
test("composePrompt appends the note with absolute paths at the end of the text", () => {
  assert.equal(
    composePrompt("これを見て", [
      "/workspace/.u7agent/uploads/a1b2c3d4e5/a.png",
      "/workspace/.u7agent/uploads/a1b2c3d4e5/b.pdf",
    ]),
    [
      "これを見て",
      "",
  ...
  assert.equal(
    composePrompt("", ["/workspace/.u7agent/uploads/a1b2c3d4e5/a.png"]),
    ["<attached_files>", "- /workspace/.u7agent/uploads/a1b2c3d4e5/a.png", "</attached_files>"].join("\n"),
  );
});

test("stripAttachedFiles and splitAttachedFiles round-trip the note", () => {
  const absolute = "/workspace/.u7agent/uploads/a1b2c3d4e5/a.png";
...
test("normalizeAttachmentPaths requires the session uploads dir and at most 10 items", () => {
  const uploadsDir = ".u7agent/uploads/a1b2c3d4e5";
  ...
  for (const value of [
    ...
    ["docs/a.png"],
    ["uploads/a.png"],
    [".u7agent/uploads/fffffffff/a.png"], // 別セッションの保存先
    ["/.u7agent/uploads/a1b2c3d4e5/a.png"],
    [""],
  ]) {
    assert.equal(
...
test("toAttachmentPath only accepts paths under the session uploads dir", () => {
  ...
  assert.equal(toAttachmentPath(uploadsDir, ".u7agent/uploads/fffffffff/a.png"), undefined);
