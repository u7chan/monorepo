...
test("projects resolve the session cwd and are reported as a root-relative path", async () => {
  ...
  const unaffiliated = await store.create();
...
test("create rejects a project that disappears while the runtime is creating the session", async () => {
  ...
  const pending = store.create({ projectId: project.id });
...
test("releaseProject aborts running sessions and keeps them as unaffiliated", async () => {
  ...
  const target = await store.create({ projectId: project.id });
  const sibling = await store.create({ projectId: other.id });
  const unaffiliated = await store.create();
...
test("resolves model and thinking level per field: request → definition → app default", async () => {
  const catalog = createAgentCatalog();
  const configured = catalog.createAgent({
    name: "定義ありエージェント",
  ...
  const fromDefinition = await store.create({ agentId: configured.id });
  ...
  await store.create({ agentId: configured.id, thinkingLevel: "high" });
  assert.deepEqual(pi.createInputs.at(-1)?.model, { provider: "stub", id: "stub-plain" });
...
test("keeps the agent snapshot of each chat after definition edits and deletes", async () => {
  ...
  const pi = createStubPi();
  const store = new SessionStore({ pi, catalog });
  const agent = catalog.createAgent({ name: "削除されるエージェント" });
  const other = catalog.createAgent({ name: "別エージェント" });
  const record = await store.create({ agentId: agent.id });
  const before = store.payload(record).agent;
  const summaryBefore = store.summary(record);

  catalog.updateAgent(agent.id, { name: "別の名前", description: "別の説明" });
  ...
  assert.equal(store.summary(record).agentId, agent.id);
...
test("model-only change keeps the effective effort, then SDK clamping wins", async () => {
  ...
  const pi = createStubPi();
  const store = new SessionStore({ pi, catalog });
  const record = await store.create({ thinkingLevel: "low" });
