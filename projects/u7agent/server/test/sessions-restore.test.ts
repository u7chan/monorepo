...
test("永続化したセッションを新しい store が復元し、続きから送信できる", async () => {
  ...
  try {
    ...
    await store1.init();
    const record = await store1.create({ agentId: "agent-zundamon" });
...
test("ツール呼び出しだけのターンを含んでも meta / 一覧 / 復元後の messageCount が一致する", async () => {
  ...
  try {
    ...
    await store1.init();
    const created = await store1.create({ agentId: "agent-zundamon" });
...
test("PI_MODELS の候補外モデルでは再開せず、フォールバック後の実効モデルを保存する", async () => {
  ...
  try {
    ...
    const store1 = createStore(storeDir, { pi: pi1, workspace, catalog });
    await store1.init();
    const record = await store1.create({ agentId: "agent-zundamon", model: { provider: "stub", id: "stub-model" } });
...
test("store がワークスペース内ならセッション作成を拒否する", async () => {
  ...
  await assert.rejects(store.create({ agentId: "agent-zundamon" }), (error: Error & { statusCode?: number }) => {
...
test("プロジェクト所属セッションは登録ディレクトリを cwd にし、作成も復元も mkdir しない", async () => {
  ...
  try {
    ...
    await store1.init();
    const record = await store1.create({ agentId: "agent-zundamon", projectId: project.id });
...
test("同一プロジェクトの別セッションは同じ作業ディレクトリを共有する", async () => {
  ...
  try {
    ...
    await store.init();
    const first = await store.create({ agentId: "agent-zundamon", projectId: project.id });
    const second = await store.create({ agentId: "agent-zundamon", projectId: project.id });
