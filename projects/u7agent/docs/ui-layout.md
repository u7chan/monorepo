...
メイン領域は チャット / 設定ページ のどちらかを出し、URL（`/` または `/settings/<section>`）がそれを決める（[frontend.md](frontend.md#url-と画面の対応)）。設定ページは `<dialog>` を被せずメイン領域に出す（サイドバーと同時に見える）。compact の詳細（エージェント / スキルの編集）だけはページの一覧から開く全画面シートにする（[compact の詳細シート](#compact-の詳細シート)）。設定ページを開いている間もチャットは mount したまま `display` だけ切るので、SSE 購読（実行中のラン）・入力中の下書きは失われない。スクロール位置の保持は約束しない（非表示中は `scrollHeight` を読めないので触らず、戻ったときに追従中なら最新へ揃える。追従が外れているときの読み位置は復元せず、先頭が出る想定。詳細は[チャットの自動追従と最下部ボタン](#チャットの自動追従と最下部ボタン)）。
...
| 遷移 | きっかけ |
| --- | --- |
| true → false | スクロール後の最下部からの距離が 48px を超えた（snap で自分が書いた位置のイベントは除く） |
| false → true | 48px 以内へ自分で戻る / ボタン押下 / 送信（`sendSeq` の増加）/ 会話の切替（`sessionId` の変化。`""` からの遷移 = 新規チャットの作成も含む） |
| true のとき | 内容・容器のサイズが変わるたびに最下部へ揃える |
...
- snap の代入で起きた `scroll` は、代入後に実際に読んだ位置（`SNAP_POSITION_EPSILON = 1` の許容つき）と一致するときだけ追従を保つ。無条件に次の 1 回を無視すると、代入とイベント配送の間にユーザーが上へ戻した操作を取りこぼし、次の更新で引き戻す
...
`client/test/chatScroll.test.ts` がしきい値の境界（ちょうど / 端数 / 内容が収まる / 空）と snap 由来の判定（位置が同じ / 上へ移動済み / 控えなし）、`ChatArea` の配線（follow で gate されている・`visible` 中は書かない・ボタンの `aria-label`）を、`client/test/chatReducer.test.ts` が `sendSeq` の増減（`localUser` だけ / `resync` / `newChat`）を固定する。スクロール・ResizeObserver・フォーカスは jsdom 無しのテストでは検証できないので、実ブラウザーで次を確認する。

- ストリーミング中に上へスクロールしても引き戻されず、ボタンが出る（ホイール / トラックパッド / `PageUp`・`↑`・`Home` / タッチ）
- 追従中はユーザー操作なしにボタンが出ない（大きな tool 出力が続く場面）
- snap の直後に上へスクロールしても引き戻されず、ボタンが出る（snap 由来の判定がユーザー操作を取りこぼさない）
...
- 設定ページを往復したときの位置（追従中は最新。外れているときの実際の挙動）
