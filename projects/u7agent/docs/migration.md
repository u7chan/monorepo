...
- 移植先: `projects/u7agent`（移植当時は `projects/_labs/pi-agent-gui`）
...
- `projects/pi-agent-gui` → `projects/u7agent`（アプリ私有ディレクトリ `.pi-agent-gui` → `.u7agent`、配布イメージ `monorepo/pi-agent-gui` → `monorepo/u7agent`）。旧名のデータは引き継がない
