...
- 一覧でカタログスキルを選ぶと**閲覧ビュー**（`CatalogSkillPanel`）を出す。保存済みの名前 / 説明 / 割り当て中（`skillIds` にこのスキルを含むエージェントの件数と名前）/ 本文を確認でき、本文は応答の `body` をそのまま出す。編集は操作行の `編集` から始め（新規作成は従来どおりフォームへ直行）、`キャンセル` で閲覧ビューへ戻る。未保存の差分があるときだけ `window.confirm` を出し、確認後は下書きを破棄する（同じスキルへ入り直すと現行のカタログから始まる）。保存は `onSelectSkill(savedId)` → `refreshCatalog()` →（compact は）シートを閉じる、の順で、閲覧ビューへ戻ってから一覧が更新される。削除は編集フォームの中だけに置く
...
チャット側では `GET /api/sessions/:id/skills` がこの 4 種類（プロジェクト / 共通 / 組み込み / エージェント定義のスキル）を優先順位つきで返し、入力補助の一覧と `/skill:` の展開（送信時に本文を取得）が同じ解決を共有する。セッション未確定の `GET /api/skills/session` は同じ解決をセッション作成前に走らせ、新規チャットのピッカーへ同じ形で返す（[api-sessions.md](api-sessions.md#get-apisessionsidskills)、[api-sessions.md](api-sessions.md#get-apiskillssession)、[persistence.md](persistence.md#スキルの扱い)）。
...
- 設定 → スキルの一覧は、編集できる catalog のスキルと同じリストに共通分と組み込み分を**読み取り専用スキル**として並べる。再読み込みはこのブロックの見出しに 1 つだけ置く（この API は両方を 1 回で返すため、グループ側に置くと片方だけを更新するように見える）。共通行と組み込み行はどちらも button で、選ぶと右ペイン / シートに本文ビューを出す（編集・削除はしない）。行の選択は行固有の `path` で持ち、名前が同じでも別の行として扱う（上書きされた組み込みは同名の共通行と並ぶ）。読み上げ名は名前行（名前 / スコープ / 版）に閉じ、説明・パス・警告は button の外のテキストにする（行全体のクリックは `after` の overlay で保つ）
- 本文ビュー（`ReadOnlySkillPanel`）は先頭の操作行（`SKILL` + スコープ名、右にタブ）+ メタ（名前 / 説明 / 置き場 / 版）+ 本文で組む。本文のコピーボタンは表示と同じ生テキスト（frontmatter 込み）をコピーし、常時表示にする（hover で隠さない）。`FilePreview` の「本文をコピー」は表示用に正規化した本文（CRLF / CR → LF、末尾の空行を落とす）なので別の契約になる（[file-preview.md](file-preview.md#コピー)）
- ファイルスキルは「本文 / ファイル」のタブを持つ。ファイルタブは `FileBrowser` を root = SKILL.md の親ディレクトリで置き、`references/` / `scripts/` / `assets/` のファイルをタブ / 行番号 / ハイライト / HTML・画像プレビュー / 全画面で開ける（[file-preview.md](file-preview.md#画面と-root)）。ダウンロード / 削除 / リネームの導線は出さない（`FileBrowser` の `readOnly`）。出す条件は `scope !== "builtin"` かつ workspace root 相対の `.../SKILL.md` の親が取れることで、判定は `client/src/lib/fileSkills.ts` の `fileSkillDir` が行う（絶対パス（POSIX / ドライブ / UNC）・空・`..` を含むパスは拒否）。組み込みは実体の無い仮想パスなのでタブを出さない
- ファイルタブは初回に開いたときだけ mount し、以降は `display` で隠して保持する（タブと選んだファイルを行き来しても取り直さない）
- ファイルスキルの本文は応答に含めない。モデルは `path` を `read` で読み、本文は `read` 時点のファイル内容になる（作成後に編集すればその内容、削除すれば読取り失敗）。設定の本文ビューも同じで、選択のたびに（同じ行を押し直したときも）`GET /api/files/preview` で `relativePath` を読み直す（256 KiB 超・バイナリ・削除済みはサンドボックスの文言をそのまま出す）。組み込みはワークスペースに実体が無いため、`body` / `version` を一覧へ載せ、本文ビューはこの `body` をそのまま出す（`body` は SKILL.md の frontmatter 込みの全文）
...
- 作成の body はキー省略を未指定として受け付け、追加項目の無い定義もそのまま使える。利用不能なモデル参照も形式が正しければ定義には保持できる（実行時に検証してエラーになる）。
...
- 作成の body はキー省略を未指定として受け付け、追加項目の無い定義もそのまま使える。
