...
- 設定 → スキルの一覧は、編集できる catalog のスキルと同じリストに共通分と組み込み分を**読み取り専用スキル**として並べる。再読み込みはこのブロックの見出しに 1 つだけ置く（この API は両方を 1 回で返すため、グループ側に置くと片方だけを更新するように見える）。共通行と組み込み行はどちらも button で、選ぶと右ペイン / シートに本文ビューを出す（編集・削除はしない）。読み上げ名は名前行（名前 / スコープ / 版）に閉じ、説明・パス・警告は button の外のテキストにする（行全体のクリックは `after` の overlay で保つ）
- ファイルスキルの本文は応答に含めない。モデルは `path` を `read` で読み、本文は `read` 時点のファイル内容になる（作成後に編集すればその内容、削除すれば読取り失敗）。設定の本文ビューも同じで、選択のたびに `GET /api/files/preview` で `relativePath` を読み直す（256 KiB 超・バイナリ・削除済みはサンドボックスの文言をそのまま出す）。組み込みはワークスペースに実体が無いため、`body` / `version` を一覧へ載せ、本文ビューはこの `body` をそのまま出す（`body` は SKILL.md の frontmatter 込みの全文）
