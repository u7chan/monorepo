...
import { fetchSessionSkills, sessionSkillsSource, type SessionSkillsState } from "../lib/sessionSkills";
 ...
 * プレビュー API を使う。取得先が変わるたびに取り直す (本文は送信時に BFF が読むため、一覧は優先順位の表示に使う)。
 */
export function useSessionSkills({
  ...
  }, [agentId, beginRequest, enabled, projectId, reloadCount, sessionId]);
