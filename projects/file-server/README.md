...
- ファイル/ディレクトリの削除・リネーム
- ディレクトリ作成
- **外部公開URL配信 (`GET /public/*`)**: 認証不要で `UPLOAD_DIR/public/` 以下のファイルを直接配信（HTML/XHTML/SVG を含む）。**trusted content 前提**であり、untrusted な HTML/JS を same-origin で安全にホストする機能ではない。
...
| メソッド | パス | 説明 |
|---------|------|------|
| GET | `/public/*` | `UPLOAD_DIR/public/` のファイルを直接配信（HTML/XHTML/SVG を含む。trusted content 前提） |

> **Trusted content 前提の注意**:
> `/public/*` の HTML/XHTML/SVG 配信は、ファイルが「明示的なユーザー操作」または「ユーザー確認付き Internal API」経由で配置された信頼済みコンテンツであることを前提とする。
> サーバー側バリデーション（`<script>`、`on*` イベント属性、`<iframe>` 等のブロック）はアップロード・更新時に適用される。
> untrusted な HTML/JS を same-origin で安全にホストする機能ではない。
