...
他のアプリケーションから file-server を保存先・配信先として利用する場合の契約。連携先の追加に file-server 側の改修は不要で、以下をそのまま使う。

#### 読み出し（公開配信）

| メソッド | パス | 説明 |
|---------|------|------|
| GET | `/public/<path>` | 認証不要。`UPLOAD_DIR/public/<path>` をそのまま配信（HTML/XHTML/SVG を含む） |

- `Content-Type` は拡張子から推定する（`mime-types`）。未知の拡張子は `application/octet-stream`
- ディレクトリは 404、ディレクトリトラバーサルは 403
- `Cache-Control` と CORS ヘッダは付与しない。`Range` リクエストには未対応（常に全量を返す）

#### 書き込み

1. `POST /login`（auth 有効時のみ必要）: `username` / `password` / `returnTo` を form で送信する。成功時は `session` Cookie が発行されてリダイレクトし、失敗時は 401 でログイン画面の HTML を返す
2. `POST /api/upload`: multipart で `files`（最大10件）と `path`（バーチャルパス）を送信する
   - `path` が既存ディレクトリ、または末尾が `/` の場合は `path/<ファイル名>`、それ以外は保存先のファイルパスとして扱う
   - 親ディレクトリは自動作成され、同名ファイルは上書きされる
   - レスポンスは `{ success, uploaded: string[], failed: { name, reason }[] }`
   - auth 無効時はログイン不要で、API リクエストは匿名（書き込み可）として扱われる

#### ヘルスチェック

| メソッド | パス | 説明 |
|---------|------|------|
| GET | `/healthz` | 認証不要。`public/`・`private/` が読み書き実行可能なら `{"status":"ok"}`、不可なら 503 |

#### 名前空間と分離の限界

- 連携先ごとに `public/<namespace>/...` の専用ディレクトリを使う（規約であり、サーバー側では強制しない）
- `public/` は誰でも読み出せ、書き込み権限を持つ資格情報は他連携先のファイルも上書き・削除できる
- 連携先ごとの quota・レート制限・監査ログ、署名付き URL やトークンによる非公開配信は未実装
- auth 有効時、`user` ロールの資格情報でも `public/` には書き込める（ロールで分離されるのは `private/` のみ）
