...
export const buttonBaseClassName =
  "inline-flex items-center justify-center gap-2 rounded-lg font-medium transition-colors focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"

const primaryToneClassName = "bg-indigo-600 text-white hover:bg-indigo-700"
...
const dangerToneClassName = "text-slate-500 hover:bg-red-50 hover:text-red-600"
...
export const rowActionToggleClassName = `${buttonBaseClassName} ${secondaryToneClassName} h-8 w-8 p-0 text-sm md:w-auto md:px-3 aria-expanded:bg-indigo-50 aria-expanded:text-indigo-700 aria-expanded:ring-indigo-300`

/** Toolbar toggle; state is driven by `aria-pressed` rather than class juggling. */
export const toggleButtonIdleClassName = `${buttonBaseClassName} ${secondaryToneClassName} ${smSizeClassName} w-full min-w-0 whitespace-nowrap sm:w-auto aria-pressed:bg-indigo-50 aria-pressed:text-indigo-700 aria-pressed:ring-indigo-300`
