...
describe("upload", () => {
	...
	it("should upload a single file to the public scope", async () => {
		...
		formData.append("files", new File([testContent], "test.txt", { type: "text/plain" }))
		...
		const savedContent = await readFile(path.join(UPLOAD_DIR, "public", "test.txt"), "utf-8")
	...
	it("should upload multiple files", async () => {
		...
		expect(res.status).toBe(200)
		const data = (await res.json()) as { success: boolean; uploaded: string[]; failed: unknown[] }
		expect(data.success).toBe(true)
		...
		expect(await readFile(path.join(UPLOAD_DIR, "public", "file1.txt"), "utf-8")).toBe("Content 1")
		expect(await readFile(path.join(UPLOAD_DIR, "public", "file2.txt"), "utf-8")).toBe("Content 2")
	})
	...
	it("should upload files to nested directory", async () => {
		...
		const formData = new FormData()
		formData.append("files", new File([testContent], "baz.txt", { type: "text/plain" }))
		...
		const savedContent = await readFile(path.join(UPLOAD_DIR, nestedPath), "utf-8")
		expect(savedContent).toBe(testContent)
	})
	...
	it("should upload files to a directory path (with trailing slash)", async () => {
		...
		const formData = new FormData()
		formData.append("files", new File([testContent], "uploaded.txt", { type: "text/plain" }))
	...
	it("should reject upload to parent directory path", async () => {
		const formData = new FormData()
		formData.append("files", new File(["content"], "evil.txt", { type: "text/plain" }))
	...
	it("should upload files via htmx and return HTML with file list", async () => {
		...
		const formData = new FormData()
		formData.append("files", new File([testContent], "htmx-test.txt", { type: "text/plain" }))
		formData.append("path", "public")
	...
	it("should upload multiple files via htmx", async () => {
		const formData = new FormData()
		formData.append("files", new File(["Content 1"], "htmx-multi1.txt", { type: "text/plain" }))
		formData.append("files", new File(["Content 2"], "htmx-multi2.txt", { type: "text/plain" }))
		formData.append("path", "public")
	...
	it("should upload files to nested directory via htmx", async () => {
		...
		const formData = new FormData()
		formData.append("files", new File([testContent], "bar.txt", { type: "text/plain" }))
