...
async function writeUsers(users: PlainUser[]) {
	const userConfigs = await Promise.all(
		users.map(async (u) => ({
			username: u.username,
			passwordHash: await Bun.password.hash(u.password, { algorithm: "bcrypt", cost: 4 }),
			role: u.role ?? "user",
		})),
	)
...
describe("POST /api/rename", () => {
	it("renames a file in the public root directory", async () => {
		...
		const body = new URLSearchParams({ path: "public/before.txt", name: "after.txt" })
		...
		expect(await readFile(path.join(UPLOAD_DIR, "public", "after.txt"), "utf-8")).toBe("content")
		expect(Bun.file(path.join(UPLOAD_DIR, "public", "before.txt")).text()).rejects.toThrow()
	})

	it("renames a file in a subdirectory", async () => {
		await mkdir(path.join(UPLOAD_DIR, "public", "parent"), { recursive: true })
		await writeFile(path.join(UPLOAD_DIR, "public", "parent", "before.txt"), "nested")

		const body = new URLSearchParams({ path: "public/parent/before.txt", name: "after.txt" })
		const res = await app.request(
			new Request("http://localhost/api/rename", {
				method: "POST",
				body,
		...
		expect(
			await readFile(path.join(UPLOAD_DIR, "public", "parent", "after.txt"), "utf-8"),
		).toBe("nested")
	})

	it("renames a directory and keeps its contents", async () => {
		await mkdir(path.join(UPLOAD_DIR, "public", "before-dir"), { recursive: true })
		await writeFile(
			path.join(UPLOAD_DIR, "public", "before-dir", "inside.txt"),
			"inside-content",
		)

		const body = new URLSearchParams({ path: "public/before-dir", name: "after-dir" })
		const res = await app.request(
			new Request("http://localhost/api/rename", {
				method: "POST",
				body,
		...
		expect(res.status).toBe(301)
		expect(res.headers.get("location")).toBe("/?path=public")
		expect(await stat(path.join(UPLOAD_DIR, "public", "after-dir"))).toMatchObject({
			isDirectory: expect.any(Function),
		})
		expect(
			await readFile(path.join(UPLOAD_DIR, "public", "after-dir", "inside.txt"), "utf-8"),
		).toBe("inside-content")
	})
	...
	it("treats same-name rename as a success no-op", async () => {
		...
		const body = new URLSearchParams({ path: "public/same.txt", name: "same.txt" })
		const res = await app.request(
			new Request("http://localhost/api/rename", {
				method: "POST",
				body,
		...
		expect(res.status).toBe(301)
		expect(res.headers.get("location")).toBe("/?path=public")
		expect(await readFile(path.join(UPLOAD_DIR, "public", "same.txt"), "utf-8")).toBe("same-content")
	})
	...
	it("returns FileNotFound when the source does not exist", async () => {
		const body = new URLSearchParams({ path: "public/missing.txt", name: "after.txt" })
		const res = await app.request(
			new Request("http://localhost/api/rename", {
				method: "POST",
				body,
	...
	it("returns AlreadyExists when the destination already exists", async () => {
		...
		expect(res.status).toBe(400)
		const json = (await res.json()) as { success: boolean; error: { name: string; message: string } }
		expect(json.success).toBe(false)
	...
	it("keeps the parent breadcrumb in nested htmx renames", async () => {
		...
		await writeFile(path.join(UPLOAD_DIR, "public", "foo", "before.txt"), "content")

		const body = new URLSearchParams({ path: "public/foo/before.txt", name: "after.txt" })
		const res = await app.request(
			new Request("http://localhost/api/rename", {
				method: "POST",
				body,
	...
	it("pre-renders rename controls in the browse view", async () => {
		await writeFile(path.join(UPLOAD_DIR, "public", "browse-test.txt"), "content")
	...
	it("allows admins to rename items in private area", async () => {
		...
		await writeFile(path.join(UPLOAD_DIR, "private", "alice", "before.txt"), "content")
		...
		expect(
			await readFile(path.join(UPLOAD_DIR, "private", "alice", "after.txt"), "utf-8"),
		).toBe("content")
	})
})
