...
- `DELETE` は履歴だけ消し、作業ディレクトリと添付は残す（誤アップロードの通常ファイル単位の削除は 設定 → ファイル からできるが、セッション単位ではフォルダを消さない）。confirm は「このセッションの履歴を削除しますか？（作業ディレクトリのファイルは残ります）実行中の処理は停止されます。」と表示する。
