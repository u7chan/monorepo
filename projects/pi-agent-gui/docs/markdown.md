...
| 1 段落で数式の判定に使う文字数 | 20000 | `inline.ts` |
| 数式 1 つの文字数 / ノード数 | 20000 / 4000 | `latex.ts` |
...
- インライン数式にする条件は 3 つすべて: ① 開き `$` の直後が空白でない ② 同じ段落内に閉じがあり、その直前が空白でない ③ 解析が成功する
- ① ② が欠けたときは地の文のまま（通貨記号と衝突させない）、③ で落ちたときだけ記法ごと原文（`literal` → `<code class="md-lit">`）にする
