...
ツール履歴の行とサマリーの右端にある位相ラベル（完了 / 実行中 / エラー）は `w-7`（28px）に固定して右寄せする。位相で文字幅が変わると（実測 完了 18 / 実行中 27 / エラー 27.42px）、左隣のコピーボタンとサマリーの truncate 境界が動くため。右寄せなのでラベルの文字の右端は固定幅でも動かない。
