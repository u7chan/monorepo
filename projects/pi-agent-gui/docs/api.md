...
エージェント定義には任意の `suggestions`（`{ label, prompt }` の配列）を持たせられます。空の会話の firstview に `label` のボタンとして並び、押すと `prompt` をそのまま送信します（セッションタイトルの元にもなる）。既定の組み込みエージェントは `agent-builder` だけが 3 件を持ちます。
