...
| チャットの添付ファイル（アップロード / 画像配信 / 注記） | `server/src/attachments.ts`、`server/src/routes/sessions.ts`、`client/src/lib/attachments.ts`、`client/src/components/Composer.tsx`、`client/src/hooks/useAgentDesk.ts` | [session-files.md](session-files.md)、[api-sessions.md](api-sessions.md) |
