...
送信（`POST /api/sessions/:id/messages`）は text と optional の images（最大 10 枚）を受け取り、モデルはそのセッションの SDK セッションが持つ実効値（`session.model`）で決まる。送信ごとのモデル指定は無いため、表示（入力欄 / ヘッダー）と実際の送信先が食い違わないよう、クライアントは選択中セッションの実効モデルだけを表示する。 `model.input` に `image` があるモデルだけ画像添付を有効にし、非対応モデルでは Composer と API の両方で画像送信を拒否する。
