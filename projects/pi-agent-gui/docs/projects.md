...
プロジェクトはワークスペース内のディレクトリの登録で、`server/src/projects.ts` の `ProjectStore` がメモリ内に持つ。`cwd` はワークスペース root 相対のパスだけを持つ（絶対パスで保存するとマウント先の変更で壊れる）。root 自身とアプリの作業ディレクトリ `<appdir>`（現 `.pi-agent-gui`）自身・配下は登録できない。
