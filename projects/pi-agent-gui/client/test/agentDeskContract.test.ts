// useAgentDesk の互換 export と返却 contract の固定。
//
// useAgentDesk は画面 (App) から見た facade で、composer / 設定ページが型と定数を import している。
// 返却値は UI が使う名前を消さないことを型で、export は実行時で確認する。
import assert from "node:assert/strict";
import test from "node:test";
import type { AgentDesk } from "../src/hooks/useAgentDesk";
...
type MissingKeys = Exclude<keyof ContractKeys, keyof AgentDesk>;
...
const { ALL_THINKING_LEVELS, effortLabel, useAgentDesk } = await import("../src/hooks/useAgentDesk");

test("keeps the facade exports used by the UI", () => {
  assert.equal(typeof useAgentDesk, "function");
