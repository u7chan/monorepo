...
test("複数対象の書き出しは 1 ファイルにまとめ、日付入りの名前を付ける", () => {
  ...
  assert.equal(fileName, "pi-desk-backup-2026-02-01.json");
...
test("単一対象の書き出しは対象名をファイル名に使う", () => {
  ...
  assert.equal(fileName, "pi-desk-definitions-2026-02-01.json");
});
...
test("書き出したファイルはそのまま読み戻せる", () => {
  ...
  assert.equal(parsed.app, "pi-desk");
...
test("data のキーの並びに関わらず、対象は定義順に返る", () => {
  const parsed = parseBackupFile(
    JSON.stringify({ app: "pi-desk", schema: 1, data: { projects: [], definitions: definitionsPayload([], []) } }),
  );
...
test("読み込めないファイルは原因の分かるメッセージで失敗する", () => {
  const cases: [string, string][] = [
    ...
    ["schema 違い", JSON.stringify({ app: "pi-desk", schema: 2, data: { definitions: {} } })],
    ["data なし", JSON.stringify({ app: "pi-desk", schema: 1 })],
    ["data が空", JSON.stringify({ app: "pi-desk", schema: 1, data: {} })],
    ["未知の対象", JSON.stringify({ app: "pi-desk", schema: 1, data: { unknown: [] } })],
  ];
