...
//   1. ボタンはソース表示のパス行 (行数の隣) に置き、reveal を渡さない (hover できる端末でも隠さない)
//   2. 渡すのは表示中の本文 (previewCopyText(code)) で、行番号やハイライトの markup は含めない
...
test("本文のコピーボタンはソース表示のパス行に置き、hover でも隠さない", () => {
  ...
  assert.ok(pathRow.includes("previewCopyText(code)"), "表示中の本文を渡していない");
  assert.ok(!pathRow.includes("reveal="), "reveal を渡すと hover できる端末で隠れる");
...
test("コピーの成功表示は表示中のタブに紐づける (タブを切り替えたら捨てる)", () => {
  const preview = read("src/components/FilePreview.tsx");
  assert.match(preview, /<FileCopyButton key=\{activePath\}/, "key を張り替えてタブごとに状態を捨てる");
});
