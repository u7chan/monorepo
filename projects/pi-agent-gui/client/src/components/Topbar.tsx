  ...
  return (
    <header className="grid gap-3 px-6 pb-3 pt-5 wide:px-8 wide:pt-6">
      <div className="flex items-start justify-between gap-4">
        <div className="text-2xs font-semibold uppercase tracking-label text-ink-ghost">LOCAL WORKSPACE</div>
        {/* 正常時のモデルと接続状態は入力欄のピッカーや画面の様子から分かるので、エラーのときだけ出す */}
        {runtimeStatus.error ? (
          <div className="inline-flex min-w-0 max-w-full items-center gap-2 rounded-full border border-danger/40 px-2.5 py-1.5 text-1xs text-danger-text">
