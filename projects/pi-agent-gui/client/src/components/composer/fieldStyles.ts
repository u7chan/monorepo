...
export const fieldLabelClass = (compact: boolean): string =>
  ["flex min-w-0 items-center text-[10px] text-ink-faint", compact ? "gap-2" : "gap-1.5"].join(" ");
...
export const selectClass = (compact: boolean): string =>
  ["py-1 pl-1.5 disabled:cursor-not-allowed disabled:opacity-55", compact ? "text-[16px]" : "text-[11px]"].join(" ");
