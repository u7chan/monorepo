...
export const BACKUP_APP = "pi-desk";
...
export function parseBackupFile(text: string): BackupFile {
  ...
  if (!isRecord(parsed) || parsed.app !== BACKUP_APP) {
    throw new Error("pi desk のバックアップファイルではありません");
  }
...
export function serializeBackup(
  ...
  const fileName = targets.length === 1 ? `pi-desk-${targets[0]}-${date}.json` : `pi-desk-backup-${date}.json`;
