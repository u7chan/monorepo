...
class ExecutionRequest(BaseModel):
    ...
    code_content: str = Field(..., description="Python code to execute", min_length=1)
    ...
    format_code: bool | None = Field(
        default=True,
        description="Whether to format code with ruff",
    )
    lint_code: bool | None = Field(
        default=True,
...
class ExecutionResult(BaseModel):
    ...
    return_code: int = Field(..., description="Exit code from code execution")
    execution_time: float = Field(..., description="Execution time in seconds", ge=0)
    formatted_code: str | None = Field(
        default=None,
        description="Code after ruff formatting",
    )
    lint_warnings: list[str] | None = Field(
        default=None,
...
class ContainerConfig(BaseModel):
    ...
    cpu_limit: str = Field(default="0.5", description="CPU limit for container")
    network_mode: str = Field(
        default="none",
        description="Network mode for container security",
    )
    user: str = Field(default="1000:1000", description="User ID for non-root execution")
